...
with col[1]:
    ...
    def create_vector_store_and_store_github_data(developer_df):
        ...
        try:
            # Convert the dataframe to a string representation (or JSON)
            developer_data_str = developer_df.to_json()

            # Create a new vector store and upload the data to it
            vector_store = client.beta.vector_stores.create(
                name="GitHub Data Vector Store",
                description="This vector store contains GitHub developer data for querying.",
                embedding_model="gpt-3.5-turbo",
                texts=[developer_data_str],
            )
    ...
    def create_openai_assistant_with_vector_store(vector_store_id):
        ...
        try:
            ...
            assistant = client.beta.assistants.create(
                ...
                model="gpt-4o",
                tools=[{"type": "vector_search"}],
                tool_resources={
                    "vector_search": {
                        "vector_store_id": vector_store_id  # Vector store ID is passed here
                    }
                }
            )

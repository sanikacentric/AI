...
export default function () {
  ...
  for (let index = 0; index < 1; index++) {
    ...
    const message = schemaRegistry.serialize({
      data: payload,
    ...
    writerCounter.add(1, { topic: "ip.documents" });

    console.log(`Produced message ${index + 1} to topic ${produceTopic}`);
  }

  sleep(10); // Wait for messages to be consumed
  ...
  reader.subscribe({
    eachMessage: (message) => {
      ...
      readerCounter.add(1, { topic: "cds.invoice" });
      readerCount++;
    },
  });
  sleep(5); // Sleep for 5 seconds to allow messages to be consumed
  reader.unsubscribe();
  ...
  /*let messages = await consumer.run({
 ...
 /*reader.subscribe({
  ...
  sleep(10); // Give the reader some time to process messages

  console.log(`Total messages read from ${consumeTopic}: ${readerCount}`);

  //const end = Date.now();
  //const duration = end - start;
  //console.log(`Test execution time: ${diff} ms`);
  //myTrend.add(duration);
  //myRate.add(duration < 500);
  //myCounter.add(1);

  //check(duration, {
  //  "duration is less than 500ms": (d) => d < 500,
  //});
}

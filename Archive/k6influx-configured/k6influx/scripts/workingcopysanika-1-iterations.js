...
const consumeTopics = ["cds.invoice","invoice.status"]; // Reader topics  "cds.invoice",
...
/*
const readers = consumeTopics.map(topic => new Reader({//new Reader({
  brokers: brokers,
  topic: topic,
  sasl: saslConfig,
  tls: tlsConfig,
...
//console.log('Before reader:', moment().tz('America/New_York').format('YYYY-MM-DD HH:mm:ss.SSS'));
const readers = [];
for (let i = 0; i < consumeTopics.length; i++) {
  console.log('Inside reader');
  //console.log('Inside reader:',moment().tz('America/New_York').format('YYYY-MM-DD HH:mm:ss.SSS'));
...
export default function () {
  ...
  for (let index = 0; index < 1; index++) {
    ...
    //console.log('Inside payload:',moment().tz('America/New_York').format('YYYY-MM-DD HH:mm:ss.SSS'));
    const payload = {
  ...
  console.log('after producing message');
  //console.log('After producing message:', moment().tz('America/New_York').format('YYYY-MM-DD HH:mm:ss.SSS'));
  sleep(4); // Sleep for 3 seconds

  console.log('after sleep');
  //console.log('after sleep:', new Date().toLocaleString());

  //let messages = readers.consume({ limit: 100 });
  let messages = readers.flatMap(reader => reader.consume({ limit: 5000 }));

  console.log('after let message');
  //console.log('after let message:', moment().tz('America/New_York').format('YYYY-MM-DD HH:mm:ss.SSS'));
  ...
  /*check(messages, {
    ...
    "key is correct": (msgs) =>
      schemaRegistry
        .deserialize({ data: msgs[1].key, schemaType: SCHEMA_TYPE_STRING })
  ...
  /*readers.forEach(reader => {
    ...
    /*check(messages, {
    "messages returned": (msgs) => msgs.length > 0,
    "key is correct": (msgs) =>msgs.every(
      schemaRegistry
        .deserialize({ data: msgs.key, schemaType: SCHEMA_TYPE_STRING })
        .length === 36), // UUID length is 36 characters
    "value is correct": (msgs) => msgs.every(
      schemaRegistry.deserialize({
        data: msgs.value,
        schemaType: SCHEMA_TYPE_JSON,
      }).messageId !== null),
  ...
  /*check(messages, {
    "messages returned": (msgs) => msgs.length > 0,
    "key is correct": (msgs) => msgs.every(msg => schemaRegistry.deserialize({ data: msg.key, schemaType: SCHEMA_TYPE_STRING }).length === 36),
    "value is correct": (msgs) => msgs.every(msg => schemaRegistry.deserialize({ data: msg.value, schemaType: SCHEMA_TYPE_JSON }).messageId !== null),
  });
  */

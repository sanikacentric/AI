...
export const options = {
  scenarios: {
    test8: {
      ...
      duration: '1s' // Run the test for 50000 seconds
    }
    },
  thresholds: {
...
let messages = []; // Initialize an array to store messages for each virtual user
export default function () {
  ...
  if (!messages[__VU]) { // Check if a message for the current virtual user exists
    ...
    messages[__VU] = { key, value }; // Store the message for the current virtual user
  }

  writer.produce({
    messages: [
      {
        key: messages[__VU].key,
        value: messages[__VU].value,
      },
    ],
  });
  ...
  let consumedMessages = readers.flatMap(reader => reader.consume({ limit: 100 }));
  console.log(`Reader consumed ${consumedMessages.length} messages`);

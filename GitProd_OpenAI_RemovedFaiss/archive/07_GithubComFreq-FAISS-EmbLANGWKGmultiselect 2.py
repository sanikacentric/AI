...
# Fetch the list of repositories from the specified organization
try:
...
repo_name = st.sidebar.selectbox("Select Repository:", options=repo_names, index=0 if repo_names else None)
...
columns_order = ['commits', 'additions', 'deletions', 'total_changes', 'commit_frequency', 
                 'commit_details', 'prs_created', 'prs_open','prs_merged', 'prs_closed', 
                 'prs_reviewed', 'code_reviews', 'issues_created', 
                 'issues_assigned', 'issues_resolved', 'commit_merge', 'affected_files']

developer_df_transposed = developer_df_transposed.reindex(columns_order)

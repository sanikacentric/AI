...
with col[1]:
    ...
    def query_openai_with_faiss(prompt):
        ...
        elif "highest additions" in prompt or "most additions" in prompt:
            max_additions = developer_df['additions'].max()
            developer_name = developer_df.loc[developer_df['additions'] == max_additions, 'Developer'].values[0]
            result_text = f"The developer with the most lines of code added is {developer_name} with {max_additions} additions."

        elif "lowest additions" in prompt or "fewest additions" in prompt:
            min_additions = developer_df['additions'].min()
            developer_name = developer_df.loc[developer_df['additions'] == min_additions, 'Developer'].values[0]
            result_text = f"The developer with the fewest lines of code added is {developer_name} with {min_additions} additions."

        elif "highest deletions" in prompt or "most deletions" in prompt:
            max_deletions = developer_df['deletions'].max()
            developer_name = developer_df.loc[developer_df['deletions'] == max_deletions, 'Developer'].values[0]
            result_text = f"The developer with the most lines of code deleted is {developer_name} with {max_deletions} deletions."

        elif "lowest deletions" in prompt or "fewest deletions" in prompt:
            min_deletions = developer_df['deletions'].min()
            developer_name = developer_df.loc[developer_df['deletions'] == min_deletions, 'Developer'].values[0]
            result_text = f"The developer with the fewest lines of code deleted is {developer_name} with {min_deletions} deletions."

        elif "most active" in prompt:
            # Most active developer is the one with the highest total changes (additions + deletions)
            max_changes = developer_df['total_chan'].max()
            developer_name = developer_df.loc[developer_df['total_chan'] == max_changes, 'Developer'].values[0]
            result_text = f"The most active developer is {developer_name} with a total of {max_changes} changes (additions + deletions)."

        elif "least active" in prompt:
            # Least active developer is the one with the fewest total changes (additions + deletions)
            min_changes = developer_df['total_chan'].min()
            developer_name = developer_df.loc[developer_df['total_chan'] == min_changes, 'Developer'].values[0]
            result_text = f"The least active developer is {developer_name} with a total of {min_changes} changes (additions + deletions)."

        elif "highest prs created" in prompt:
            max_prs_created = developer_df['prs_create'].max()
            developer_name = developer_df.loc[developer_df['prs_create'] == max_prs_created, 'Developer'].values[0]
            result_text = f"The developer with the most PRs created is {developer_name} with {max_prs_created} PRs."

        elif "lowest prs created" in prompt or "fewest prs created" in prompt:
            min_prs_created = developer_df['prs_create'].min()
            developer_name = developer_df.loc[developer_df['prs_create'] == min_prs_created, 'Developer'].values[0]
            result_text = f"The developer with the fewest PRs created is {developer_name} with {min_prs_created} PRs."

        elif "highest prs merged" in prompt:
            max_prs_merged = developer_df['prs_merge'].max()
            developer_name = developer_df.loc[developer_df['prs_merge'] == max_prs_merged, 'Developer'].values[0]
            result_text = f"The developer with the most PRs merged is {developer_name} with {max_prs_merged} PRs."

        elif "lowest prs merged" in prompt or "fewest prs merged" in prompt:
            min_prs_merged = developer_df['prs_merge'].min()
            developer_name = developer_df.loc[developer_df['prs_merge'] == min_prs_merged, 'Developer'].values[0]
            result_text = f"The developer with the fewest PRs merged is {developer_name} with {min_prs_merged} PRs."

        elif "highest prs closed" in prompt:
            max_prs_closed = developer_df['prs_closed'].max()
            developer_name = developer_df.loc[developer_df['prs_closed'] == max_prs_closed, 'Developer'].values[0]
            result_text = f"The developer with the most PRs closed is {developer_name} with {max_prs_closed} PRs."

        elif "lowest prs closed" in prompt or "fewest prs closed" in prompt:
            min_prs_closed = developer_df['prs_closed'].min()
            developer_name = developer_df.loc[developer_df['prs_closed'] == min_prs_closed, 'Developer'].values[0]
            result_text = f"The developer with the fewest PRs closed is {developer_name} with {min_prs_closed} PRs."

        # Code Reviews Queries
        elif "highest code reviews" in prompt:
            max_code_reviews = developer_df['code_reviews'].max()
            developer_name = developer_df.loc[developer_df['code_reviews'] == max_code_reviews, 'Developer'].values[0]
            result_text = f"The developer with the most code reviews is {developer_name} with {max_code_reviews} reviews."

        elif "lowest code reviews" in prompt or "fewest code reviews" in prompt:
            min_code_reviews = developer_df['code_reviews'].min()
            developer_name = developer_df.loc[developer_df['code_reviews'] == min_code_reviews, 'Developer'].values[0]
            result_text = f"The developer with the fewest code reviews is {developer_name} with {min_code_reviews} reviews."

        # Issues Queries
        elif "highest issues created" in prompt:
            max_issues_created = developer_df['issues_created'].max()
            developer_name = developer_df.loc[developer_df['issues_created'] == max_issues_created, 'Developer'].values[0]
            result_text = f"The developer with the most issues created is {developer_name} with {max_issues_created} issues."

        elif "highest issues resolved" in prompt:
            max_issues_resolved = developer_df['issues_resolved'].max()
            developer_name = developer_df.loc[developer_df['issues_resolved'] == max_issues_resolved, 'Developer'].values[0]
            result_text = f"The developer with the most issues resolved is {developer_name} with {max_issues_resolved} resolved issues."

        elif "highest issues assigned" in prompt:
            max_issues_assigned = developer_df['issues_assigned'].max()
            developer_name = developer_df.loc[developer_df['issues_assigned'] == max_issues_assigned, 'Developer'].values[0]
            result_text = f"The developer with the most issues assigned is {developer_name} with {max_issues_assigned} issues."

        # Affected Files Queries
        elif "highest affected files" in prompt:
            max_affected_files = developer_df['affected_files'].apply(len).max()
            developer_name = developer_df.loc[developer_df['affected_files'].apply(len) == max_affected_files, 'Developer'].values[0]
            result_text = f"The developer who affected the most files is {developer_name} with {max_affected_files} affected files."

        # Commit Merge Queries
        elif "highest commit merge" in prompt:
            max_commit_merge = developer_df['commit_merge'].max()
            developer_name = developer_df.loc[developer_df['commit_merge'] == max_commit_merge, 'Developer'].values[0]
            result_text = f"The developer with the most commits merged is {developer_name} with {max_commit_merge} commits merged."

        # PR Open Queries
        elif "highest prs open" in prompt:
            max_prs_open = developer_df['prs_open'].max()
            developer_name = developer_df.loc[developer_df['prs_open'] == max_prs_open, 'Developer'].values[0]
            result_text = f"The developer with the most open PRs is {developer_name} with {max_prs_open} open PRs."

        # Total PRs Queries
        elif "highest total prs" in prompt:
            max_total_prs = developer_df['total_prs'].max()
            developer_name = developer_df.loc[developer_df['total_prs'] == max_total_prs, 'Developer'].values[0]
            result_text = f"The developer with the most total PRs is {developer_name} with {max_total_prs} PRs."

        # Custom handling for queries asking for the number of developers and their details
        elif "how many developers" in prompt or "count of developers" in prompt or "total developers" in prompt or "details of each" in prompt or "its information" in prompt:
            # Count the total number of developers
            total_devs = len(developer_data)
    
            if total_devs > 0:
               developer_list = []
               for dev, data in developer_data.items():
                   dev_info = f"{dev}: {data['commits']} commits, {data['additions']} additions, {data['deletions']} deletions, " \
                        f"{data['commit_frequency']} commits/day, {data['prs_created']} PRs created, " \
                        f"{data['prs_merged']} PRs merged, {data['prs_closed']} PRs closed, " \
                        f"{data['prs_open']} PRs open, {data['commit_merge']} commits merged, " \
                        f"{data['code_reviews']} code reviews, {data['issues_created']} issues created, " \
                        f"{data['issues_assigned']} issues assigned, {data['issues_resolved']} issues resolved."
                   developer_list.append(dev_info)

            # Concatenate all developer details
               result_text = f"There are {total_devs} developers. Here are their details:\n\n" + "\n\n".join(developer_list)
            else:
               result_text = "There are no developers in the current dataset."

...
with col[1]:
    ...
    def create_vector_store_and_store_github_data(developer_df):
        """
        This function creates a vector store to store the GitHub developer data
        and returns the vector store ID.
        """
        ...
        try:
            ...
            upload_response = client.files.create(
                purpose='user_data',  # You might need to adjust the purpose depending on your use case
                file=open("/tmp/developer_data.json", "rb")
            )

            file_id = upload_response.id # Get the file ID from the upload response

            # Create a new vector store using the uploaded file's ID
            vector_store = client.beta.vector_stores.create(
                name="GitHub Data Vector Store",
                file_ids=[file_id]
            )
            vector_store_id = vector_store.id  # Assign the vector_store_id globally
            st.write(f"Vector store created successfully with ID: {vector_store_id}")  # Feedback to user
    ...
    def create_openai_assistant_with_vector_store(vector_store_id):
        ...
        with access to the vector store containing GitHub data.
        """
        try:
            ...
            assistant = client.beta.assistants.create(
                ...
                tool_resources={
                    "file_search": {
                        "vector_store_ids": [vector_store_id]  # Vector store ID is passed here
                    }
                }
            )
            st.write("Assistant with vector store created successfully!")  # Feedback to user
        except Exception as e:
            st.error(f"Failed to create assistant: {e}")


    # Button to create the vector store and OpenAI assistant
    if st.sidebar.button("Generate Insights from Vector Store"):
        try:
            # Create the vector store and store the GitHub data if vector_store_id is not set
            if vector_store_id is None:
                vector_store_id = create_vector_store_and_store_github_data(developer_df)

            if vector_store_id:
                st.write(f"Using Vector Store ID: {vector_store_id}")
                # Create the assistant with access to the vector store
                create_openai_assistant_with_vector_store(vector_store_id)
            else:
                st.error("Vector store creation failed. Cannot proceed with assistant creation.")
        except Exception as e:
    ...
    # Ensure that the vector store is initialized before querying
    if vector_store_id is None:
        st.error("Vector store is not initialized. Please generate the insights first.")
    else:
        # Proceed with querying the assistant
        st.subheader("Developer Productivity Bot")
        user_input2 = st.text_input("Ask a question about the developer productivity data:")
        ...
        if st.button("Ask Assistant"):
            if user_input2:
                ...
                st.write(f"Using Vector Store ID: {vector_store_id}")  # Debug info
                # Query the vector store directly for developer insights
                chat_completion = client.chat.completions.create(
                    ...
                    messages=[
                        ...
                        {"role": "user", "content": f"Query the vector store and provide the correct response based on this data: {user_input2}. \
                        Use the GitHub developer data in the vector store to give an answer. Structure your answer in the following format: \
                        1. **Summary**: Provide an overview of the answer. \
                        2. **Details**: Analyze the data stored in the vector database to provide a detailed response. \
                        3. **Developer-Specific Insights**: Highlight the specific data for developers from the vector store. \
                        4. **Correct Answer**: Provide a clear answer based on the vector data. \
                        5. **Conclusion**: Summarize the findings."}
                    ],
                    tool_resources={
                        "vector_search": {
                            "vector_store_id": vector_store_id,  # Ensure this value is set properly
                            "query": user_input2
                        }
                    }
                )
